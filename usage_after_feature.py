...
query_users = 'SELECT DATE(FROM_UNIXTIME(login_timestamp)) AS date, COUNT(*) AS no_of_logins FROM login_history where date(from_unixtime(login_timestamp)) between "2018-06-02" and "2019-02-02" GROUP BY date ;'
...
plt.title('Daily Average Users After Feature')
plt.xlabel('Date')
